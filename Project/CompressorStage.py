...
class CompressorStage(om.ExplicitComponent):
    def initialize(self):
    ...
    def setup(self):
        ...
        self.add_input('N', desc='rpm')
        self.add_input('thickness', desc='blade thickness max')
        self.add_input('b_s', desc='chord')
        self.add_input('b_r', desc='chord')
        ...
        self.add_output('n_poly', desc="")
        self.add_output('M_out', desc="")
        # self.add_output('chi_2r', desc="")
    ...
    def compute(self, inputs, outputs):
        ...
        M_out = cz/(np.cos(alp3)*np.sqrt(gamma*R*T3))

        chi1 = bet1 + 0
        delta_r = (bet2-chi1)/(np.sqrt(sigma_r)/0.25-1)
        chi_2r = bet2 + delta_r
        # print('Chi2r = {0} for P01 = {1}'.format(chi_2r, P01))
...
if __name__ == "__main__":
    ...
    prob.model.add_subsystem('C1', CompressorStage(), promotes_inputs=['*'], promotes_outputs=['*'])
